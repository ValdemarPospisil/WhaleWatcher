...
class ImageDetailViewModelTest {
    ...
    @Test
    fun `loadDetails success emits Success state with repository info and tags`() = runTest {
        ...
        val mockTagsResponse = DockerTagsResponse(
            count = 1,
    ...
    @Test
    fun `loadDetails failure on details API emits Error state`() = runTest {
        coEvery { repository.getImageDetails("library", "nginx") } returns Result.failure(RuntimeException("Network error"))
    ...
    @Test
    fun `loadDetails failure on tags API emits Error state`() = runTest {
        val mockRepoInfo = RepositoryInfo(name = "nginx", namespace = "library")
        coEvery { repository.getImageDetails("library", "nginx") } returns Result.success(mockRepoInfo)
        coEvery { repository.getImageTags("library", "nginx") } returns Result.failure(RuntimeException("Tags error"))
