...
class SearchViewModelTest {
    ...
    @Test
    fun `search input debounces by 500ms before making api call`() = runTest {
    ...
    @Test
    fun `fetching next page appends results and updates state`() = runTest {
        val mockResponse1 = DockerSearchResponse(
            count = 2,
            next = "page=2",
            results = listOf(DockerSearchResult(repoName = "test-image-1"))
        )
        val mockResponse2 = DockerSearchResponse(
            count = 2,
            next = null,
    ...
    @Test
    fun `empty state is emitted when no results found`() = runTest {
