...
2. **Branch per Track:** Before starting work on a new track, create a new branch with a name corresponding to the track's mission (e.g., `feature/docker-search`) and switch to it.
3. **Strict Test-Driven Development (TDD):** Implement all features and fixes using the Red-Green-Refactor cycle. No application code should be written without a failing test.
